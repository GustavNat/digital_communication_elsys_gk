...
/* RPi PIN ASSIGNMENTS */
#define MISO1 9    // ADC 1 MISO (GPIO pin number)
...
#define SPI_SS 5   // GPIO for slave select (GPIO 15).
#define CLK 11      // GPIO for SPI clock (GPIO 16).
/* END RPi PIN ASSIGNMENTS */

#define BITS 10            // Bits per sample.
...
int main(int argc, char *argv[])
{
    ...
    while (sample < num_samples) {
        ...
        while ((now_reading != reading) && (sample < num_samples)) {
            ...
            for (i=0; i < ADCS; i++) {
                val[sample*ADCS+i] = (rx[i*2] << (BITS - 8)) + (rx[(i*2)+1] >> (16 - BITS));
            }
